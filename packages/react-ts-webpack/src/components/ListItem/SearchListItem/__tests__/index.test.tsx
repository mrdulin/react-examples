...
describe('component - SearchListItem test suites with test utils', () => {
  ...
  describe('util tests', () => {
    ...
    it('renders text correctly', () => {
      component = TestUtils.renderIntoDocument<IProps>(<SearchListItem item={book} onClick={() => {}} />);
      dom.title = TestUtils.findRenderedDOMComponentWithTag(component, 'h2');
      dom.description = TestUtils.findRenderedDOMComponentWithTag(component, 'p');
      ...
      expect((dom.image as React.HTMLProps<HTMLImageElement>).src).toBe(book.image);
    ...
    it('triggers click event correctly', (done: jest.DoneCallback) => {
      ...
      const clickCallback = () => done();
      component = TestUtils.renderIntoDocument<IProps>(<SearchListItem item={book} onClick={clickCallback} />);

      dom.root = TestUtils.findRenderedDOMComponentWithClass(component, 'listItem');
