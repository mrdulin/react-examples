...
export default class Toutiao extends React.Component {
  ...
  render() {
    ...
    const tabDom = types.map((type, index) => {
      return <Tab key={type.key + '-' + index} style={{color: 'rgb(0, 188, 212)', WebkitTapHighlightColor: 'rgba(0, 0, 0, 0.1)', width: '50px'}}
                  label={type.name} value={index}/>
    })
      ...
      slide = types.map((type, index) => {
        ...
        if (data.length) {
          return <div className='slide' key={type.key}>
            <List>
              {data.map((news, idx) => <ListItem key={news.uniquekey || (index + '-' + idx)} leftAvatar={<Avatar src={news.thumbnail_pic_s}/>}
    ...
    return <div id='toutiao'>
      <Tabs className="tabs" ref={ref => this._tabs = ref} inkBarStyle={{width: '50px'}} style={{overflowX: 'auto'}}
            tabItemContainerStyle={{backgroundColor: '#fff', display: 'block'}} onChange={this.handleChange} value={slideIndex}>{tabDom}</Tabs>
